...
-->
    ...
    <xsl:template name="getNavTitle">
        ...
        <!-- 截断标题，最多显示32个字符 -->
        <xsl:choose>
            <xsl:when test="string-length($navTitle) > 32">
                <xsl:value-of select="concat(substring($navTitle, 1, 32), '...')"/>
            </xsl:when>
            <xsl:otherwise>
...
-->
    ...
    <xsl:template match="*[contains(@class,' topic/keyword ')]" name="topic.keyword">
        <xsl:apply-templates select="." mode="inlineTextOptionalKeyref">
            <xsl:with-param name="copyAttributes" as="element()">
                <wrapper xsl:use-attribute-sets="keyword"/>
            </xsl:with-param>
        </xsl:apply-templates>
    </xsl:template>
    ...
    <xsl:template match="*[contains(@class,' topic/cite ')]">
        <xsl:apply-templates select="." mode="inlineTextOptionalKeyref">
            <xsl:with-param name="copyAttributes" as="element()">
                <wrapper xsl:use-attribute-sets="cite"/>
            </xsl:with-param>
        </xsl:apply-templates>
    </xsl:template>
